...
# 
# Purpose ----
# To fit the theoretical economic model to the dataset. The fitting is 
# required to identify the numeric values of the parameters, which
# cannot variate (unlike the 9 variables provided as input).
# **********************************************************************
...
# Calculate the skewness (measure of assymetry) before the Yeo-Johnson
# transform
...
# Purpose: Ensures that each numeric attribute has a mean value of 0 and a standard
# deviation of 1. This is done by combining the scale data transform (divide each
# value by the standard deviation) and the centre data transform (subtract the mean
# from each value).
# BEFORE
summary(data_normalize_transform)
...
# Manual Model Performance Comparison ----
# summary(model)
# print(model)
...
# If you wish to use the 20% data for a more accurate test/evaluation of the model
# predictions <- predict(model, test)

## RMSE ----
# rmse <- sqrt(mean((test$qs_stddev - predictions)^2))
# print(paste("RMSE =", rmse))

## SSR ----
# SSR is the sum of squared residuals (the sum of squared differences
# between observed and predicted values)
# ssr <- sum((test$qs_stddev - predictions)^2)
# print(paste("SSR =", ssr))

#### SST ----
# SST is the total sum of squares (the sum of squared differences
# between observed values and their mean)
# sst <- sum((test$qs_stddev - mean(test$qs_stddev))^2)
# print(paste("SST =", sst))
...
# absolute_errors <- abs(predictions - test$qs_stddev)
