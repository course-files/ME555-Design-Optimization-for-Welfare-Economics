...
# Objective Function
def objective(r, cw, T, ce):
    return r ** -1.0 - (math.sin(cw / T) + ce)
...
range_space = np.array([objective(r, cw, T, ce) 
                        for r, cw, ce in zip(r_design_space, cw_design_space,
                                             ce_design_space)])
...
# Print the Result
print('Optimum: f(r=%.5f, cw=%.5f, ce=%.5f) = %.5f' % 
      (r_design_space[optimum_index], cw_design_space[optimum_index], 
       ce_design_space[optimum_index], range_space[optimum_index]))